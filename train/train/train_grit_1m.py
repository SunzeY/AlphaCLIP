...
class CLIP_Clean_Train():
    def __init__(self, local_rank=0, lr=4e-5, weigth_decay=0.02, log_scale=4.6052, lora_rank=-1, common_pair=0.0, para_gamma=0.01, exp_name="auto", warmup_length=200, epoch_num=1, subnum=10000):
        ...
        for i, (images, masks, texts) in enumerate(tqdm(dataloader, disable=(dist.get_rank() != 0))):
            ...
            if step % 100 == 0:
                if dist.get_rank() == 0:
                    ...
                    if step % 100 == 0 and step != 0 and step > 800:
    ...
    def train(self, common_pair=False, resume=False, amp=False, warmup_length=200):
        ...
        for epoch in range(start_epoch, self.num_epoch):
            # train
            if (trainset.__len__() * epoch) > 4000 * self.batch_size * 256: # for 64 only
                print(trainset.__len__(), epoch)
                break
